...
const OPEN_WEATHER_LOCAL_BASE = '*/openweather';
...
export const weatherHandlers = [
  ...
  http.get(`${OPEN_WEATHER_LOCAL_BASE}/weather`, async ({ request }) => {
    const url = new URL(request.url);
    const cityName = resolveCityName(url.searchParams.get('q'));
    const units = url.searchParams.get('units');
  ...
  http.get(`${OPEN_WEATHER_LOCAL_BASE}/forecast`, async ({ request }) => {
    const url = new URL(request.url);
    const cityName = resolveCityName(url.searchParams.get('q'));
    const units = url.searchParams.get('units');
